...
(function () {
  ...
  if (new URLSearchParams(location.search).get('demo') === '1') {
    ...
    setInterval(function () { meters += 100; render({meters: meters, car_key: 'demo'}); }, 2000);
    return;
  }
  KappsOdometer.start(render, {version: 1, cars: {}});
})();
`;
const script = [read('src/odometer.js'), read('src/renderer.js'), boot].join('\n');
if (script.includes('</script')) throw Error('Unexpected closing script tag in source');
const html = read('src/template.html').replace('<!-- SCRIPTS -->',
