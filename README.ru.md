...
[Скачать v0.4.0](https://github.com/AT-Vadim/kapps-odometer/releases/download/v0.4.0/Odometer-Kapps-v0.4.0.zip) · [English documentation](README.md)

## Установка

1. Скачайте **Odometer-Kapps-v0.4.0.zip** в разделе **Releases** этого репозитория.
...
- Без видимых статусов; по умолчанию тёмный фон с непрозрачностью 70%.
- Настраиваемые цвет и opacity фона, автоматическое масштабирование под окно Kapps.
...
В блоке «Оформление одометра» выберите один из четырёх стилей. Выбор сохраняется
...
- **iRacing · классический** — палитра таблицы из референса: тёмная панель,
  жирные светлые цифры, жёлтая полоса и выделенная цифра 100 м. Без статусов.
  При выборе устанавливается тёмный фон, а opacity сохраняется.
...
- **Механический · колёсики** — колёсики без подписей. Светлое колёсико **100 m**
...
<img src="docs/iracing.png" alt="Одометр в стиле iRacing" width="370">
...
По умолчанию непрозрачность внешнего фона — 70%; цвет и opacity настраиваются. Системная настройка уменьшения
...
## Цвет, прозрачность и размер окна

1. Временно замените URL Custom Overlay **внутри Kapps** на
   `http://127.0.0.1:8182/Odometer/?manage=1`.
2. Активируйте Racing Overlay, увеличьте область через **Edit** и нажмите **Apply**,
   как описано ниже в разделе «Как сделать страницу настроек кликабельной в Kapps». В блоке **Оформление одометра**
   выберите **Цвет фона** палитрой или введите HEX-код. Установите
   **Непрозрачность фона** от **0% (прозрачный)** до **100% (непрозрачный)**.
3. Верните URL `http://127.0.0.1:8182/Odometer/` и измените размер окна в Kapps.

Предпросмотр обновляется
сразу, настройка сохраняется автоматически и применяется к открытым виджетам
того же профиля Kapps примерно за секунду. **Сбросить фон** возвращает исходный
тёмный цвет текущего стиля и 70%, сохраняя выбранный стиль. Меняется только фон панели;
цифры и поверхности механических колёсиков сохраняют свою непрозрачность.

Размер окна одометра меняйте **в самом Kapps**. Одометр автоматически впишется
в выделенную ширину и высоту, увеличиваясь или уменьшаясь без искажения пропорций.
Он располагается по центру; при несовпадении пропорций свободное место прозрачно.
Масштабируется **только вид одометра**. Страница статистики и настроек остаётся
обычной страницей с прокруткой.

## Как сделать страницу настроек кликабельной в Kapps

`?manage=1` открывает настройки одометра, но передачей кликов управляет Kapps.
Кнопка **Edit** включает перемещение и изменение размера. **Apply** завершает
этот режим и разрешает клики внутри страницы, пока Racing Overlay активно.

1. В Kapps → Racing Overlay → ваш **Odometer Custom Overlay** временно замените
   URL на `http://127.0.0.1:8182/Odometer/?manage=1`.
2. При использовании **Open in a Window** переключитесь через **Alt+Tab** на
   окно **Racing Overlay**, а не на основные настройки Kapps или iRacing.
   На виджетах появятся кнопки **Edit**.
3. Нажмите **Edit** на одометре и потяните край или угол, увеличив область примерно
   до **800 × 850 px** или доступного размера экрана. Затем нажмите **Apply**.
   Теперь можно нажимать список стилей, палитру, ползунок и прокручивать страницу.
   Пока включён **Edit**, элементы внутри страницы не получают клики.
4. Выберите **iRacing · классический**. Стиль устанавливает тёмный фон `#232633`,
   сохраняя opacity. Жёлтые акценты — `#FFCB00`, светлые цифры — `#EEEEEE`,
   текст выделенной десятичной цифры — `#171821`. Фон по-прежнему можно менять;
   **Сбросить фон** для этого стиля возвращает тёмный фон и 70%.
   Перед закрытием дождитесь надписи **Сохранено**.
5. Верните URL `http://127.0.0.1:8182/Odometer/`. Снова активируйте Racing Overlay,
   через **Edit → изменение размера → Apply** верните размер одометра, затем
   переключитесь в iRacing. Одометр автоматически впишется в выделенную область.

В экспериментальном режиме **Open in iRacing** сначала нажмите **Ctrl+K** при
активном окне iRacing, чтобы включить режим ввода Kapps. Затем используйте
**Edit → изменение размера → Apply**, как описано выше. После настройки снова
нажмите **Ctrl+K**, чтобы выйти из режима ввода. Это сочетание относится именно
к Open in iRacing; для обычного **Open in a Window** оно не требуется.

Если клики проходят насквозь, снова выберите Racing Overlay через **Alt+Tab**.
Если вместо нажатия элементов двигается окно, сначала нажмите **Apply**.
Клик вне виджетов может снова снять фокус с Kapps. Сам одометр не может отменить
пропуск кликов на уровне окна, в которое он встроен.

Шаги сверены с обработкой фокуса, **Edit/Apply** и ввода iframe в установленном
**Kapps 1.24.38**. В других версиях поведение может отличаться. Настраивайте
одометр в том же профиле Kapps: обычные Chrome/Edge имеют отдельные данные.
...
Следуйте разделу «Как сделать страницу настроек кликабельной в Kapps»:
активируйте Racing Overlay, увеличьте область через **Edit** и нажмите **Apply**. **Скачать копию (Download backup)** скачивает
JSON, **Импорт JSON (Import JSON)** восстанавливает его. Затем верните обычный URL.
...
**v0.4.0.** Протокол сверён с Kapps **1.24.38**, подключение к серверу проверено.
Проверены расчёт, смена машин, фильтры повторов/гаража/эвакуации, сохранение,
защита от двойного подсчёта, импорт, сохранение оформления, прозрачность фона
и масштабирование всех четырёх стилей. Полная проверка во время
